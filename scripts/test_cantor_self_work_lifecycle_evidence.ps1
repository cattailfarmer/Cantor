...
$verifier = Join-Path $PSScriptRoot 'verify_cantor_self_work_lifecycle_evidence.ps1'
$manifestPath = Join-Path $root 'experiments/self_work_lifecycle_admission_p0/artifacts/self_work_lifecycle_evidence_manifest.json'
& $verifier -ManifestPath $manifestPath | Out-Null
$temporaryRoot = Join-Path ([IO.Path]::GetTempPath()) "cantor-swl-evidence-$([guid]::NewGuid().ToString('N'))"
...
try {
    ...
    $missing.artifacts = @($missing.artifacts | Where-Object { $_.path -ne 'crates/cantor_core/src/self_work_lifecycle.rs' })
...
Write-Output 'self_work_lifecycle_evidence_tests_passed positive=1 refusals=2 cleanup=true'

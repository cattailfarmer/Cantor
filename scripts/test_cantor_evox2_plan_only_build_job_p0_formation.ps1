...
$verify = Join-Path $PSScriptRoot 'verify_cantor_evox2_plan_only_build_job_p0_formation.ps1'
& $verify -Root $root

$parent = [IO.Path]::GetFullPath('D:\CantorBuilds')
$testRoot = Join-Path $parent ('evox2-plan-only-formation-' + [guid]::NewGuid().Guid)
...
try {
    ...
    $manifestSource = Join-Path $root 'experiments/evox2_plan_only_build_job_p0/formation_evidence_manifest.json'
    $manifest = Get-Content -LiteralPath $manifestSource -Raw | ConvertFrom-Json
    $manifestDestination = Join-Path $testRoot 'experiments/evox2_plan_only_build_job_p0/formation_evidence_manifest.json'
    ...
    $scriptDestination = Join-Path $testRoot 'scripts/verify_cantor_evox2_plan_only_build_job_p0_formation.ps1'
    ...
    $spec = Join-Path $testRoot 'specifications/Cantor_EVO_X2_Plan_Only_Build_Job_P0.sop'
    ...
    Copy-Item -LiteralPath (Join-Path $root 'specifications/Cantor_EVO_X2_Plan_Only_Build_Job_P0.sop') -Destination $spec -Force

    $mutated = Get-Content -LiteralPath $manifestDestination -Raw | ConvertFrom-Json
    $mutated.verification.physical_build_authorized = $true
    ...
    $mutated = Get-Content -LiteralPath $manifestDestination -Raw | ConvertFrom-Json
    $mutated.verification.remote_root = 'C:/AI/services/cantor-current-pilot-48479932'
    $mutated | ConvertTo-Json -Depth 20 -Compress | Set-Content -LiteralPath $manifestDestination -NoNewline
    $refused = $false
    try { & $scriptDestination -Root $testRoot | Out-Null } catch { $refused = $true }
    if (-not $refused) { throw 'completed-root substitution admitted' }

    'cantor_evox2_plan_only_build_job_p0_formation_tests=passed isolated_successes=1 isolated_refusals=3'
} finally {

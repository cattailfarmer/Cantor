...
try {
  cargo test -p cantor_ecosystem --test self_work_update_broker_b1_cdrive_production_preparation_commission_proposal --locked --offline
  if ($LASTEXITCODE -ne 0) { throw "focused debug tests failed" }

  $env:CARGO_PROFILE_RELEASE_OVERFLOW_CHECKS = "true"
  cargo test -p cantor_ecosystem --test self_work_update_broker_b1_cdrive_production_preparation_commission_proposal --release --locked --offline
  if ($LASTEXITCODE -ne 0) { throw "focused release tests failed" }

  cargo build -p cantor_ecosystem --bin cantor-self-work-update-broker-b1-cdrive-production-preparation-commission-proposal --bin cantor-self-work-update-broker-b1-cdrive-production-preparation-commission-proposal-evidence-verify --locked --offline
  if ($LASTEXITCODE -ne 0) { throw "focused CLI build failed" }

  $evidence = Join-Path $repositoryRoot "experiments/self_work_update_broker_b1_cdrive_production_preparation_commission_proposal_p0/implementation_provider_free_evidence"
  $cases = @(
    @{
      Executable = Join-Path $repositoryRoot "target/debug/cantor-self-work-update-broker-b1-cdrive-production-preparation-commission-proposal.exe"
      Argument = Join-Path $evidence "request.json"
      Expected = Get-Content -LiteralPath (Join-Path $evidence "proposal.json") -Raw
    },
    @{
      Executable = Join-Path $repositoryRoot "target/debug/cantor-self-work-update-broker-b1-cdrive-production-preparation-commission-proposal-evidence-verify.exe"
      Argument = $evidence
  ...
  Write-Output "production_preparation_commission_proposal_provider_free=passed"
} finally {
  Pop-Location
}

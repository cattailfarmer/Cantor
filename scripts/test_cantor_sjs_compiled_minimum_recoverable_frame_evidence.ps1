...
param(
    ...
    [string]$CargoTargetDirectory = 'D:\CantorBuilds\cantor-mrf-p0-focused-script',
...
function Test-RetainedMrfEvidence([string]$ArtifactDirectory) {
    ...
    $retainedOutput = Invoke-CantorCargoBinary 'cantor-sjs-minimum-recoverable-frame-verify' $retainedBundleJson
    ...
    if ($verification.status -cne 'minimum_recoverable_frame_verified_provider_free' -or
        $verification.input_class -cne 'synthetic_provider_free_fixture' -or
        $verification.job_count -ne 2 -or
        $verification.hint_count -ne 8 -or
        $verification.mandatory_hint_count -ne 3 -or
        $verification.recovery_source_count -ne 2 -or
        $verification.initial_basis_count -ne 8 -or
        $verification.final_basis_count -ne 4 -or
        $verification.attempt_count -ne 6 -or
        $verification.admitted_release_count -ne 4 -or
        $verification.drift_refusal_count -ne 1 -or
        $verification.underdetermined_refusal_count -ne 1 -or
        $verification.narrative_event_count -ne 6 -or
        $verification.locally_irreducible -ne $true -or
        $verification.pass_budget_exhausted -ne $false -or
...
if ($VerifyExisting) {
    ...
    Test-RetainedMrfEvidence $outputFullPath
    Write-Output "sjs_compiled_minimum_recoverable_frame_evidence_passed mode=verify_existing files=4 replay=2 retained_replay=1 jobs=2 hints=8 mandatory=3 sources=2 basis=8..4 attempts=6 admitted=4 drift=1 underdetermined=1 narrative=6 local_minimum=true effects=0 output=$outputFullPath"
    return
}
...
$bundleOutput = Invoke-CantorCargoBinary 'cantor-sjs-minimum-recoverable-frame-fixture' ''
$bundleBody = Remove-OneLineTerminator $bundleOutput 'fixture CLI output'
$verificationOutput = Invoke-CantorCargoBinary 'cantor-sjs-minimum-recoverable-frame-verify' $bundleOutput
...
$temporaryDirectory = Join-Path $outputParent ('.cantor-mrf-evidence-' + [guid]::NewGuid().ToString('N'))
...
Test-RetainedMrfEvidence $outputFullPath
Write-Output "sjs_compiled_minimum_recoverable_frame_evidence_passed mode=materialize files=4 replay=2 retained_replay=1 jobs=2 hints=8 mandatory=3 sources=2 basis=8..4 attempts=6 admitted=4 drift=1 underdetermined=1 narrative=6 local_minimum=true effects=0 output=$outputFullPath"

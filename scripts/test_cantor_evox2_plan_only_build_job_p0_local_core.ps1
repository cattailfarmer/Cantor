...
$verifyRelative = 'scripts/verify_cantor_evox2_plan_only_build_job_p0_local_core.ps1'
$verify = Join-Path $root $verifyRelative
& $verify -Root $root

$parent = [IO.Path]::GetFullPath('D:\CantorBuilds')
$testRoot = Join-Path $parent ('evox2-plan-only-local-core-' + [guid]::NewGuid().Guid)
...
try {
    ...
    $manifestRelative = 'experiments/evox2_plan_only_build_job_p0/local_core_evidence_manifest.json'
    ...
    $moduleRelative = 'crates/cantor_core/src/evox2_plan_only_build_job.rs'
    ...
    'cantor_evox2_plan_only_build_job_p0_local_core_tests=passed isolated_successes=1 isolated_refusals=3'
} finally {

...
$verifier = Join-Path $PSScriptRoot 'verify_cantor_self_work_update_handoff_evidence.ps1'
$manifestPath = Join-Path $root 'experiments/self_work_update_handoff_admission_p0/artifacts/self_work_update_handoff_evidence_manifest.json'
& $verifier -ManifestPath $manifestPath | Out-Null
$temporaryRoot = Join-Path ([IO.Path]::GetTempPath()) "cantor-swu-evidence-$([guid]::NewGuid().ToString('N'))"
...
try {
    ...
    $missing.artifacts = @($missing.artifacts | Where-Object { $_.path -ne 'crates/cantor_ecosystem/src/workspace_admission/update_handoff.rs' })
...
Write-Output 'self_work_update_handoff_evidence_tests_passed positive=1 refusals=2 cleanup=true'

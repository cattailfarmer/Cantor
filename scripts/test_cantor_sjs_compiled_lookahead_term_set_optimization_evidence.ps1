...
param(
    ...
    [string]$CargoTargetDirectory = 'D:\CantorBuilds\cantor-lookahead-term-optimization-p0-evidence',
...
if (-not $VerifyExisting) {
    ...
    $bundleJson = Invoke-CantorBinary 'cantor-sjs-compiled-lookahead-term-set-fixture' $null
...
$verificationJson = Invoke-CantorBinary 'cantor-sjs-compiled-lookahead-term-set-verify' $retainedBundleJson
$verification = $verificationJson | ConvertFrom-Json
if ($verification.status -cne 'verified_provider_free_exact_pool' -or
    [long]$verification.candidate_count -ne 8 -or
...
Write-Output "sjs_compiled_lookahead_term_set_evidence_passed mode=$mode profile=$profile files=4 replay=2 candidates=8 obligations=6 edges=12 subsets=92 selected=3 rejected=5 dominated=1 uncovered=0 execution_authorized=false effects=0 output=$outputRoot"

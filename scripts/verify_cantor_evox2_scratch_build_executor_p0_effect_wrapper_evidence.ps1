...
if (@($manifest.artifacts).Count -ne 46) { throw 'effect-wrapper evidence artifact count differs' }
$seen = @{}
...
$moduleRaw = Get-Content -LiteralPath (Join-Path $rootPath 'crates/cantor_core/src/evox2_scratch_build_deployment_effect.rs') -Raw
$cliRaw = Get-Content -LiteralPath (Join-Path $rootPath 'crates/cantor_core/src/bin/cantor-evox2-scratch-build-live-evidence-verify.rs') -Raw
$testsRaw = Get-Content -LiteralPath (Join-Path $rootPath 'crates/cantor_core/tests/evox2_scratch_build_deployment_effect.rs') -Raw
foreach ($required in @('Evox2ScratchBuildRemoteProbeResult', 'Evox2ScratchBuildRemotePreflightObservation', 'compile_evox2_scratch_build_remote_preflight_from_observation', 'openssh_native_bounded_single_call')) { if ($moduleRaw -cnotmatch [regex]::Escape($required)) { throw "live preflight observation compiler surface differs: $required" } }
if ($cliRaw -cnotmatch 'preflight-observation' -or $testsRaw -cnotmatch 'remote_probe_and_observation_forms_are_strict_and_replayable' -or $testsRaw -cnotmatch 'remote_preflight_observation_refuses_process_output_and_outcome_drift' -or $testsRaw -cnotmatch 'fresh_cli_compiles_only_a_strict_live_preflight_observation') { throw 'live preflight observation compiler evidence differs' }
foreach ($forbidden in @('std::process', 'std::fs', 'std::env', 'TcpStream', 'Command::new', 'powershell.exe', 'ssh.exe', 'scp.exe')) { if ($moduleRaw.Contains($forbidden)) { throw "effect-wrapper provider-free boundary differs: $forbidden" } }
$expected = @{ artifact_count = 46; focused_tests = 18; stage_count = 10; effectful_stage_count = 7; checked_fixture_forms = 5; observation_compiler_cases = 10; isolated_successes = 1; isolated_refusals = 5; provider_requests = 0; remote_calls = 0; effects = 0 }
...
"cantor_evox2_scratch_build_effect_wrapper_evidence_verified=true artifacts=46 focused_tests=18 stages=10 effectful_stages=7 fixture_forms=5 observation_compiler_cases=10 isolated_successes=1 isolated_refusals=5 provider_requests=0 remote_calls=0 execution_authorized=false effects=0 exact_gates=$([bool] $manifest.verification.exact_workspace_gates_passed)"

...
param(
    ...
    [string]$CargoTargetDirectory = 'D:\CantorBuilds\cantor-rmp-p0-focused-script',
...
function Invoke-CantorCargoBinary([string]$Binary, [string]$InputText) {
    ...
    if ($start.PSObject.Properties.Name -contains 'StandardErrorEncoding') {
...
function Test-RetainedMaterializationPlanEvidence([string]$ArtifactDirectory) {
    ...
    $retainedOutput = Invoke-CantorCargoBinary 'cantor-runtime-closure-materialization-plan-verify' $retainedBundleJson
    $retainedBody = Remove-OneLineTerminator $retainedOutput 'retained verifier CLI output'
    if ([string]$retainedBundle.verification_file -cne "$retainedBody`n") {
    ...
    if ($verification.status -cne 'materialization_plan_compiled_effectless_verified_awaiting_separate_commission' -or
        $verification.input_class -cne 'synthetic_provider_free_fixture' -or
        $verification.upstream_root_count -ne 2 -or
        $verification.upstream_material_node_count -ne 5 -or
        $verification.upstream_producer_edge_count -ne 3 -or
        $verification.upstream_prerequisite_count -ne 2 -or
        $verification.phase_count -ne 9 -or
        $verification.operation_kind_count -ne 13 -or
        $verification.operation_count -ne 25 -or
        $verification.unresolved_operation_count -ne 25 -or
        $verification.capability_denial_count -ne 25 -or
        $verification.receipt_zero_field_count -ne 10 -or
...
if ($VerifyExisting) {
    ...
    Test-RetainedMaterializationPlanEvidence $outputFullPath
    Write-Output "runtime_closure_materialization_plan_evidence_passed mode=verify_existing files=4 replay=2 retained_replay=1 nodes=5 edges=3 prerequisites=2 phases=9 operation_kinds=13 operations=25 unresolved=25 denials=25 receipt_zero=10 effects=0 output=$outputFullPath"
    return
}
...
$bundleOutput = Invoke-CantorCargoBinary 'cantor-runtime-closure-materialization-plan-fixture' ''
$bundleBody = Remove-OneLineTerminator $bundleOutput 'fixture CLI output'
$verificationOutput = Invoke-CantorCargoBinary 'cantor-runtime-closure-materialization-plan-verify' $bundleOutput
...
$temporaryDirectory = Join-Path $outputParent ('.cantor-runtime-materialization-plan-evidence-' + [guid]::NewGuid().ToString('N'))
...
Test-RetainedMaterializationPlanEvidence $outputFullPath
Write-Output "runtime_closure_materialization_plan_evidence_passed mode=materialize files=4 replay=2 retained_replay=1 nodes=5 edges=3 prerequisites=2 phases=9 operation_kinds=13 operations=25 unresolved=25 denials=25 receipt_zero=10 effects=0 output=$outputFullPath"

...
fn main() -> ExitCode {
    ...
    if arguments.len() != 1 {
        eprintln!(
            "usage: cantor-self-work-update-broker-b1-cdrive-production-broker-evidence-verify <evidence-directory>"
        );
        return ExitCode::from(2);
    }
    match verify_b1_cdrive_production_broker_evidence_directory(Path::new(&arguments[0])).and_then(
        |verification| {

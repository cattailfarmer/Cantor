...
fn main() -> ExitCode {
    ...
    if arguments.len() != 1 {
        eprintln!(
            "usage: cantor-self-work-update-broker-b1-cdrive-production-preparation-plan <request-file>"
        );
        return ExitCode::from(2);
    }
    match read_request(Path::new(&arguments[0])).and_then(|machine_form| {
        let request = from_b1_cdrive_production_preparation_request_machine_form(&machine_form)
            .map_err(|error| error.to_string())?;
        let plan = compile_b1_cdrive_production_preparation_plan(&request)
            .map_err(|error| error.to_string())?;
        to_b1_cdrive_production_preparation_plan_machine_form(&request, &plan)
            .map_err(|error| error.to_string())
    }) {
        Ok(plan) => {
            println!("{plan}");
